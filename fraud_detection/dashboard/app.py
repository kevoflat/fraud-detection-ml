...
MODEL_PATH   = BASE / "models" / "xgb_fraud.pkl"
SCALER_PATH  = BASE / "models" / "scaler.pkl"
FEATURE_PATH = BASE / "data"   / "features.parquet"
...
def load_summary_stats():
    if FEATURE_PATH.exists():
        ...
        grp = pd.DataFrame({"hour": hours, "sum": fraud_rates * 100,
                             "count": 10000, "fraud_rate": fraud_rates})
...
def score_batch(df_raw, threshold):
    ...
    scaler   = load_scaler()
    probs = [
        float(pipeline.predict_proba(preprocess_txn(row.to_dict(), scaler))[0, 1])
        for _, row in df_raw.iterrows()
    ]
    df_out = df_raw.copy()
    df_out["fraud_probability"] = probs
    df_out["prediction"]  = ["FRAUD" if p >= threshold else "LEGIT" for p in probs]
    df_out["risk_tier"]   = pd.cut(df_out["fraud_probability"],
                                    bins=[0, 0.3, 0.6, 1.0],
...
with st.sidebar:
    ...
    if not DEMO_MODE:
        ...
        st.warning("Demo mode - model not found")
    st.markdown("---")
...
if DEMO_MODE:
    st.info("Demo mode - synthetic data shown. Add model artifacts to enable live scoring.")
st.markdown("---")
...
with tab1:
    ...
    roc_path  = BASE / "models" / "plots" / "roc_pr.png"
    conf_path = BASE / "models" / "plots" / "confusion_matrix.png"
    if roc_path.exists():
        ec1.image(str(roc_path), caption="ROC and PR curves", use_column_width=True)
    if conf_path.exists():
        ec2.image(str(conf_path), caption="Confusion matrix", use_column_width=True)

with tab2:
    st.subheader("Global SHAP feature importance")
    s1, s2 = st.columns(2)
    shap_bar_path = BASE / "models" / "plots" / "shap_bar.png"
    shap_sum_path = BASE / "models" / "plots" / "shap_summary.png"
    if shap_bar_path.exists():
        s1.image(str(shap_bar_path), caption="Mean SHAP importance", use_column_width=True)
    if shap_sum_path.exists():
        s2.image(str(shap_sum_path), caption="SHAP beeswarm", use_column_width=True)
    st.markdown("---")
        ...
        if uploaded:
            ...
            if st.button("Score all", type="primary"):
                ...
                st.metric("Fraud detected", f"{fraud_found:,}",
                          delta=f"{fraud_found/len(df_scored)*100:.2f}%",
                          delta_color="inverse")
                st.dataframe(
